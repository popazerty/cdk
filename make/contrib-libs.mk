...
$(D)/graphlcd: $(D)/bootstrap $(D)/libfreetype $(D)/libusb @DEPENDS_graphlcd@
	...
	cd @DIR_graphlcd@ && \
		...
		$(MAKE) -j$(MAKE_JOBS) all DESTDIR=$(targetprefix) && \
		@INSTALL_graphlcd@
	@CLEANUP_graphlcd@
	touch $@
...
$(D)/libcap: $(D)/bootstrap @DEPENDS_libcap@
	...
	cd @DIR_libcap@ && \
		...
		$(MAKE) -j$(MAKE_JOBS) -C libcap LIBATTR=no INCDIR=$(targetprefix)/usr/include LIBDIR=$(targetprefix)/usr/lib install
	@CLEANUP_libcap@
	touch $@
