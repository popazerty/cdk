...
#
tools-clean:
	- $(MAKE) -j$(MAKE_JOBS) -C $(appsdir)/tools distclean
...
$(D)/tools: $(appsdir)/tools/config.status
	 $(MAKE) -j$(MAKE_JOBS) -C $(appsdir)/tools all prefix=$(targetprefix) DRIVER_TOPDIR=$(driverdir) \
	CPPFLAGS="\
	...
	" && \
	 $(MAKE) -j$(MAKE_JOBS) -C $(appsdir)/tools install prefix=$(targetprefix) DRIVER_TOPDIR=$(driverdir)
	touch $@
